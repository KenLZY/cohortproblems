...
async function findOneByCode(code, with_staffs = false) {
  try {
    const [rows, fieldDefs] = await db.pool.query(
      `
            SELECT code FROM ${tableName} WHERE code = ?`,
      [code]
    );
    var list = [];
    for (let row of rows) {
...
async function get_counts() {
  try {
    const [rows, fieldDefs] = await db.pool.query(`
        SELECT ${tableName}.code as code,
            count(work.id) as count
        FROM ${tableName}
        INNER JOIN work ON work.code = ${tableName}.code
        GROUP BY ${tableName}.code
    `);
    var list = [];
    for (let row of rows) {
      let dept = { code: row.code, count: row.count };
      list.push(dept);
    }
    return list;
  } catch (error) {
