...
function NewStaffBar({
  ...
  for (let i in depts) {
    if (depts[i].code === code) {
      rows.push(
        <option value={depts[i].code} key={depts[i].code} selected>
          {depts[i].code}
        </option>
      );
    } else {
      rows.push(
        <option value={depts[i].code} key={depts[i].code}>
          {depts[i].code}
        </option>
      );
    }
...
function Staff() {
  ...
  async function initDepts() {
    // this function fetches all department codes
    // transforms it into a JSON file
